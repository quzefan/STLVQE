        ...
        const int height, const int width, scalar_t h, scalar_t w)
{
  ...
  // scalar_t lh = h - h_low;
  // scalar_t lw = w - w_low;
  scalar_t lh = h_low - h + 1;
  scalar_t lw = w_low - w + 1;
  scalar_t rh = h_high - h + 1;
  scalar_t rw = w_high - w + 1;
  // scalar_t hh = static_cast<scalar_t>(1.) - lh, hw = static_cast<scalar_t>(1.) - lw;
  scalar_t w1 = (static_cast<scalar_t>(1.) + lh) * (static_cast<scalar_t>(1.) + lw);
  scalar_t w4 = (static_cast<scalar_t>(1.) - rh) * (static_cast<scalar_t>(1.) - rw);
  scalar_t w2 = (static_cast<scalar_t>(1.) + lh) * (static_cast<scalar_t>(1.) - rw);
  scalar_t w3 = (static_cast<scalar_t>(1.) - rh) * (static_cast<scalar_t>(1.) + lw);
        ...
        scalar_t *data_col)
{
  CUDA_KERNEL_LOOP(index, n)
  {
    ...
    const int b_col = floor(index / (num_channels * width * height));
    const int c_col = floor((index - (b_col * width * height * num_channels)) / (width * height));
    const int h_col = floor((index - (b_col * width * height * num_channels) - (c_col * width * height)) / width);
    const int w_col = index - (b_col * width * height * num_channels) - (c_col * width * height) - (h_col * width);
    ...
    scalar_t *data_col_ptr = data_col + (b_col * num_channels * height_col * kernel_h * width_col * kernel_w) + (c_col * height_col * kernel_h * width_col * kernel_w) + h_col * kernel_h * width_col * kernel_w + w_col * kernel_w;
    // const scalar_t *data_im_ptr = data_im + (b_col * num_channels + c_im) * height * width;
    const scalar_t *data_im_ptr = data_im + (b_col * num_channels * height * width) + (c_col * height * width);
    const scalar_t *data_offset_ptr = data_offset + (b_col * 2 * kernel_h * kernel_w * height * width);
    // const scalar_t *data_mask_ptr = data_mask + (b_col * deformable_group + deformable_group_index) * kernel_h * kernel_w * height_col * width_col;
    // printf("offset:%f", data_offset[99 * width + 99]);
    for (int i = 0; i < kernel_h; ++i)
      for (int j = 0; j < kernel_w; ++j)
      {
        const int data_offset_h_ptr = (i * kernel_w + j) * height * width + h_col * width + w_col;
        const int data_offset_w_ptr = (i * kernel_w + j + kernel_h * kernel_w) * height * width + h_col * width + w_col;
        // const int data_mask_hw_ptr = ((i * kernel_w + j) * height_col + h_col) * width_col + w_col;
        const scalar_t offset_h = data_offset_ptr[data_offset_h_ptr];
        const scalar_t offset_w = data_offset_ptr[data_offset_w_ptr];
        // if(c_col == 1 && h_col == 150 && w_col == 150){
        //   printf("data_im_ptr:%f ", data_im_ptr[h_col * width + w_col]);
        // }
        // const scalar_t mask = data_mask_ptr[data_mask_hw_ptr];
        scalar_t val = static_cast<scalar_t>(0.);
        const scalar_t h_im = h_in + i * dilation_h + offset_h;
        const scalar_t w_im = w_in + j * dilation_w + offset_w;
        // if(h_col == 150 && w_col == 150){
        //   printf("him:%f w_im:%f", h_im, w_im);
        // }
        // printf("hin:%d,him:%f,offset_h:%f",h_in,h_im,offset_h);
        if (h_im > -1 && w_im > -1 && h_im < height && w_im < width){
          val = modulated_deform_conv2d_im2col_bilinear(data_im_ptr, width, height, width, h_im, w_im);
        }
        // if(b_col == 0 && c_col == 1 && h_col == 50 && w_col == 50){
        //   printf("val:%f ", val);
        // }
        scalar_t *data_ptr = (data_col_ptr + i * width_col * kernel_w + j);
        *data_ptr = val;
      }
  }
}
...
at::Tensor modulated_deform_conv2d_forward_cuda(
  ...
  const int channels_out = input.size(1);
  const int channels_kernel = input.size(1);
  ...
  at::Tensor output=at::zeros({batch, channels_out * kernel_h * kernel_w, height_out, width_out},input.options());
  // resize temporary columns
  at::Tensor columns =at::zeros({batch, channels_out, height_out * kernel_h, width_out * kernel_w},input.options());
  ...
  // divide into group
  output = output.view({batch, channels_out * kernel_h * kernel_w, height_out, width_out});
  ...
  modulated_deform_conv2d_im2col_cuda(
      input, offset, batch, channels, height, width, height_out,
      width_out, kernel_h, kernel_w, pad_h, pad_w, stride_h, stride_w,
      dilation_h, dilation_w, deformable_group, columns);
  columns = columns.view({batch, channels, height_out * kernel_h, width_out * kernel_w});
  ...
  // output = output.view({batch, channels * kernel_h * kernel_w, height_out, width_out});
  // output = output.transpose(1, 2).contiguous();
  // output = output.view({batch, channels_out, height_out * kernel_h, width_out * kernel_w});
  // if (with_bias) {
