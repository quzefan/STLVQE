...
class BasicRestorer(BaseModel):
    ...
    def forward_train(self, lq, gt, meta=None, save_image=False, save_path=None, iteration=None):
        ...
        qp_value = []
        losses = dict()
        output1 = self.generator(lq, qp_value)
        loss_pix1 = self.pixel_loss(output1, gt[:,1:])
        losses['loss_pix'] = loss_pix1

        outputs = dict(
            losses=losses,
            num_samples=len(gt.data),
            results=dict(lq=lq.cpu(), gt=gt.cpu(), output=output1.cpu()))
        return outputs
