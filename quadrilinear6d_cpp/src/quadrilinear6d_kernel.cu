...
__global__ void QuadriLinearForward(const int nthreads, const float* luts, const float* tri_index, const float* weight, const float* image1, const float* image2, const float* image3, const float* image4, const float* image5, const float* image6, float* output, const int luts_num, const int dim, const int shift, const float binsize, const int width, const int height, const int batch) {
        ...
        float a = image1[index_batch*height*width + index_height*width + index_width];
        float b = image2[index_batch*height*width + index_height*width + index_width];
        float c = image3[index_batch*height*width + index_height*width + index_width];
        float d = image4[index_batch*height*width + index_height*width + index_width];
        float e = image5[index_batch*height*width + index_height*width + index_width];
        float f = image6[index_batch*height*width + index_height*width + index_width];
        // printf("a:%d ", a);
        int interval = floor(binsize);
        ...
        int output_index = (index_batch*height*width+ index_height*width + index_width);

        output[output_index] = luts[a_id*interval*interval*interval*interval*interval + b_id * interval*interval*interval*interval + c_id * interval*interval*interval + d_id * interval*interval + e_id * interval + f_id];

    }
}


int QuadriLinearForwardLaucher(const float* luts, const float* tri_index, const float* weight, const float* image1, const float* image2, const float* image3, const float* image4, const float* image5, const float* image6, float* output, const int luts_num, const int luts_dim, const int shift, const float binsize, const int width, const int height, const int batch) {
    const int kThreadsPerBlock = 512;
    const int output_size = height * width * batch;
    cudaError_t err;


    QuadriLinearForward<<<(output_size + kThreadsPerBlock - 1) / kThreadsPerBlock, kThreadsPerBlock, 0>>>(output_size, luts, tri_index, weight, image1, image2, image3, image4, image5, image6, output, luts_num,  luts_dim, shift, binsize, width, height, batch);
...
int quadrilinear6d_forward_cuda(torch::Tensor luts, torch::Tensor tri_index, torch::Tensor weight, torch::Tensor image1, torch::Tensor image2, torch::Tensor image3, torch::Tensor image4, torch::Tensor image5, torch::Tensor image6, torch::Tensor output,
                           int luts_num, int luts_dim, int shift, float binsize, int width, int height, int batch)
{
    ...
    float * image_flat5 = image5.data<float>();
    float * image_flat6 = image6.data<float>();
    float * output_flat = output.data<float>();

    QuadriLinearForwardLaucher(luts_flat, tri_index_flat, weight_flat, image_flat1, image_flat2, image_flat3, image_flat4, image_flat5, image_flat6, output_flat, luts_num, luts_dim, shift, binsize, width, height, batch);
...
int quadrilinear6d_backward_cuda(torch::Tensor luts, torch::Tensor tri_index, torch::Tensor weight, torch::Tensor weight_grad, torch::Tensor image1, torch::Tensor image2, torch::Tensor image3, torch::Tensor image4, torch::Tensor output_grad,
