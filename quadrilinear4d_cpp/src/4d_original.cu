...
__global__ void QuadriLinearForward(const int nthreads, const float* luts, const float* tri_index, const float* weight, const float* image1, const float* image2, const float* image3, const float* image4, float* output, const int luts_num, const int dim, const int shift, const float binsize, const int width, const int height, const int batch) {
        ...
        int id0 = (a_id * interval * interval * interval + b_id * interval * interval + c_id * interval + d_id);
        // printf("interval:%d ", interval);
        int id1 = ((a_id + index_o1_1) * interval * interval * interval + (b_id + index_o1_2) * interval * interval + (c_id + index_o1_3) * interval + (d_id + index_o1_4));
        int id2 = ((a_id + index_o2_1) * interval * interval * interval + (b_id + index_o2_2) * interval * interval + (c_id + index_o2_3) * interval + (d_id + index_o2_4));
        int id3 = ((a_id + index_o3_1) * interval * interval * interval + (b_id + index_o3_2) * interval * interval + (c_id + index_o3_3) * interval + (d_id + index_o3_4));
        int id4 = ((a_id + 1) * interval * interval * interval + (b_id + 1) * interval * interval + (c_id + 1) * interval + (d_id + 1));
        // printf("id4:%d ", id4);
        ...
        output[output_index] = w0 * luts[id0] + w1 * luts[id1] + w2 * luts[id2] + w3 * luts[id3] + w4 * luts[id4];
        // printf("weight:%f ", w0+w1+w2+w3+w4);
        // output[output_index] = luts[id4];
        // printf("lut1:%f ", luts[1]);
        // printf("lut2:%f ", luts[2]);
            // }
    }
}
